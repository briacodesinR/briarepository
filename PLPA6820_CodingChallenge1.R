...
dataframe.zdf <- data.frame(z,zlog)
dataframe.zdf # just testing to make sure it loads correctly!
# Change the column names in your new dataframe to equal “zvec” and “zlogic”
colnames(dataframe.zdf) <- c("zvec", "zlogic")
# Make a new column in your dataframe equal to zvec squared (i.e., z2). Call the new column zsquared. 
dataframe.zdf$zsquared <- ((dataframe.zdf$zvec)^2)
dataframe.zdf
...
subset(dataframe.zdf, zsquared>10 & zsquared<100)
dataframe.zdf

dataframe.zdf[dataframe.zdf$zsquared>10 & zsquared<100,]

# Subset the zdf dataframe to only include the values on row 26
subset(dataframe.zdf [26,])
